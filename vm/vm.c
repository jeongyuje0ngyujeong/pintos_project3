								  ...
								  struct supplemental_page_table *src UNUSED)
{
	...
	while (hash_next(&i))
	{
		...
		{
			struct aux *file_aux = malloc(sizeof(struct aux));
            file_aux->file = page->file.file;
            file_aux->ofs = page->file.ofs;
            file_aux->page_read_bytes = page->file.page_read_bytes;
            file_aux->page_zero_bytes = page->file.page_zero_bytes;
            if (!vm_alloc_page_with_initializer(page_get_type(page), page->va, page->writable, NULL, file_aux))
                goto err;
            struct page *file_page = spt_find_page(dst, page->va);
...
void spt_remove_func(struct hash_elem *elem, void *aux UNUSED)
{
